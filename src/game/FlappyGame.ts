...
/** The course: how tall the gaps are, how often pipes come, and how fast they travel. */
export interface Course { gap: number; spawnEvery: number; speed: number }
export const DEFAULT_COURSE: Course = { gap: 210, spawnEvery: 2.1, speed: 132 };
...
export class FlappyGame {
  ...
  constructor(seed: number, private onDeath: (cause: string) => void, readonly course: Course = DEFAULT_COURSE) { this.rng = new SeededRandom(seed); this.reset(seed); }
  ...
  private spawn(first = false) { const gap = this.course.gap; this.pipes.push(new Pipe(first ? this.width - 50 : this.width + 80, this.rng.range(86, this.height - 86 - gap), gap)); }
  update(dt: number) {
    if (!this.running) return;
    this.elapsed += dt; this.spawnTimer += dt; this.bird.update(dt);
    if (this.spawnTimer >= this.course.spawnEvery) { this.spawnTimer -= this.course.spawnEvery; this.spawn(); }
    for (const pipe of this.pipes) {
      pipe.update(dt, this.course.speed);
